...
### 参加者の登録 【決定: 自己登録型】
参加者自身がメールOTPでサインインし、プログラムに参加する方式。

- フロー: メールでOTPサインイン → 役割選択(参加者/支援者) → ニックネーム・都道府県(任意) → `profiles` 作成、参加者コード(U001形式)を自動採番して表示。
- 実名は一切入力しない(現行の登録UXを踏襲)。
- **悪用・列挙対策(自己登録型の必須セット)**:
  - コード採番は Edge Function または DB トリガーで行い、連番の推測だけでは他者データに到達できない(RLSが本防壁)。
  - `director` ロールは自己登録で選択**不可**。ディレクター付与は Supabase 管理画面または Edge Function 経由のみ。
  - `supporter` ロールの自己登録を許すかは運用判断。当面は許可するが、プログラム紐づけ・データ閲覧はディレクターの承認(profiles の承認フラグ等)を挟む余地を残す。
  - OTP送信はSupabase標準のレート制限に加え、必要なら Edge Function 側でも制限。
...
- **TEST1〜7 デモモードは v2 で廃止**【決定】。画面確認はステージングの Supabase プロジェクト or テストユーザーで行う。`TEST*` 分岐コードは実装時に撤去。
...
- **supporter 自己登録の悪用**: 承認フラグ等の運用ガードを Phase 2 で検討(§3)。
...
### 次アクション
1. ~~未決3点の確定~~ → **完了(§12)**。
2. **Phase 0**: deny-all RLS を適用(漏洩即封鎖)。SQL草案 → `migrations/000_phase0_deny_all.sql`。
3. **Phase 1**: 新スキーマ→RLS→Auth配線→XSS一掃→TEST*撤去。migrations と rollback を同梱。
